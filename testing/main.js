...
function generateQRCodeBlobs(member) {
  ...
  const basePrefilledUrl = `${CONFIG.ATTENDANCE_FORM_URL}?usp=pp_url` +
    ...
    `&${getEntryId('PHONE')}=${encodeURIComponent(member.phone || '')}` + // Include phone number
    `&${getEntryId('FULL_NAME')}=${encodeURIComponent(member.englishName)}` + // Use full name
    `&${getEntryId('ICARE')}=${encodeURIComponent(member.iCare)}`;

  const prefilledUrlTaipei = `${basePrefilledUrl}&${getEntryId('LOCATION')}=Taipei`;
  const prefilledUrlZhongli = `${basePrefilledUrl}&${getEntryId('LOCATION')}=Zhongli`;
